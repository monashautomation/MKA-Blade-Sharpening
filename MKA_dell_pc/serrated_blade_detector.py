...
class SerratedBladeAnalyzer:
    ...
    def detect_edges(self, canny_low=100, canny_high=200):
    ...
    def find_blade_contours(self):
        ...
        self.blade_contours = [cnt for cnt in contours if cv2.contourArea(cnt) > 10]
        return self.blade_contours

    # def detect_blade_and_grinder(self, sampling_step=1, min_run=5):
    #     """
    #     STRICT blade + grinder detection
    #     - Uses run-length filtering
    #     - Enforces minimum thickness
    #     - Stabilizes Y
    #     """
    #
    #     blade_edge = []
    ...
    #
    #         # ---- BLADE EDGE (strict) ----
    #         # Find first stable run of white pixels
    #         runs = np.split(idx, np.where(np.diff(idx) != 1)[0] + 1)
    #         valid_runs = [r for r in runs if len(r) >= min_run]
    #
    #         if not valid_runs:
    #             continue
    #
    #         blade_x = valid_runs[0][0]
    #         blade_edge.append((blade_x, y))
    #
    #         # ---- GRINDER EDGE (strict) ----
    #         grinder_runs = [
    #             r for r in valid_runs
    #             if r[0] > int(self.width * 0.65)
    #         ]
    #
    #         if grinder_runs:
    #             # TRUE edge = leftmost pixel of thickest run
    #             thickest = max(grinder_runs, key=len)
    #             grinder_candidates.append((thickest[0], y))
    #
    #     self.blade_edge_points = np.array(blade_edge) if blade_edge else None
    #     self.grinder_edge_points = np.array(grinder_candidates) if grinder_candidates else None
    #
    #     # ---- FINAL GRINDER TIP (VERY STRICT) ----
    #     if self.grinder_edge_points is not None and len(self.grinder_edge_points) > 10:
    #         # Use median instead of mean (removes outliers)
    #         x_vals = self.grinder_edge_points[:, 0]
    #         y_vals = self.grinder_edge_points[:, 1]
    #
    #         median_x = int(np.median(x_vals))
    #         close = np.abs(x_vals - median_x) < 3
    #
    #         self.grinder_tip = (
    #             median_x,
    #             int(np.median(y_vals[close]))
    #         )
    #     else:
    #         self.grinder_tip = None
    #
    ...
    #     grinder_candidates = []
    #
    #     # Precompute horizontal gradient
    #     grad_x = cv2.Sobel(self.gray, cv2.CV_32F, 1, 0, ksize=3)
    #
    #     for y in range(0, self.height, sampling_step):
    #         row_bin = self.binary[y]
    #         row_grad = grad_x[y]
    #
    #         white_idx = np.where(row_bin > 100)[0]
    #         if len(white_idx) == 0:
    #             continue
    #
    #         # ---- Blade edge (same logic, stable) ----
    #         blade_edge.append((white_idx[0], y))
    #
    #         # ---- Grinder edge: gradient-based ----
    #         search = white_idx[white_idx > int(self.width * 0.65)]
    #         if len(search) == 0:
    #             continue
    #
    #         # Find strongest edge (ignore glare)
    #         grads = row_grad[search]
    #         edge_idx = search[np.argmin(grads)]  # strongest dark→bright
    #
    #         # Reject weak gradients (glare)
    #         if abs(row_grad[edge_idx]) < 5:
    #             continue
    #
    #         grinder_candidates.append((edge_idx, y))
    #
    #     self.blade_edge_points = np.array(blade_edge) if blade_edge else None
    #     self.grinder_edge_points = np.array(grinder_candidates) if grinder_candidates else None
    #
    #     # ---- Robust final grinder tip ----
    #     if len(grinder_candidates) > 20:
    #         xs = self.grinder_edge_points[:, 0]
    #         ys = self.grinder_edge_points[:, 1]
    #
    #         median_x = int(np.median(xs))
    #         mask = np.abs(xs - median_x) < 2
    #
    #         self.grinder_tip = (
    #             median_x,
    #             int(np.median(ys[mask]))
    #         )
    #     else:
    #         self.grinder_tip = None
    #
    ...
    def generate_grinding_coordinates(self, pixels_per_mm=1.0):
        ...
        for tooth in self.teeth_profiles:
            ...
            grinding_coords.append({
                ...
                'move_y_pixels': float(move_y_px),
                'move_x_mm': round(move_x_px / pixels_per_mm, 3),
                'move_y_mm': round(move_y_px / pixels_per_mm, 3),
