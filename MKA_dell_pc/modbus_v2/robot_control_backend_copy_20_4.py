...
class SerratedBladeAnalyzer:
    ...
    def _get_closest_pair_stats(self, sharp_threshold=150.0):
        ...
        while i < len(self.teeth_profiles) - 1:
            ...
            if next_i >= len(self.teeth_profiles): break
        ...
        while i < len(self.teeth_profiles) - 1:
            ...
            if move_y_mm > 1:
                ...
                if dist < min_distance:
                    min_distance   = dist
                    closest_valley = {'valley_x':valley_x,'valley_y':valley_y,'move_x_mm':move_x_mm,'move_y_mm':move_y_mm,'between_teeth':f"{ct.tooth_id}-{nt.tooth_id}",'distance_mm':dist}
...
#         x_smooth = ndimage.gaussian_filter1d(x_coords, sigma=3)
#
#         # ── Step 1: mask fully-white artifact rows ────────────────────────────
#         valid_mask = np.ones(len(y_coords), dtype=bool)
#         if self.image is not None:
#             img_array = np.array(self.image) if not isinstance(self.image, np.ndarray) else self.image
#             gray = cv2.cvtColor(img_array, cv2.COLOR_BGR2GRAY) if img_array.ndim == 3 else img_array
...
#                     valid_mask[i] = False
#
#         # ── Step 2: local rolling-median baseline ─────────────────────────────
#         # Instead of a single global mean (which breaks when the blade is tilted
#         # or the teeth are not aligned on a constant X), we compute a *local*
#         # baseline at every point using a wide rolling median window.
#         #
#         # The baseline follows the slow drift of the edge (tilt, curve) while
#         # ignoring the sharp tooth peaks/valleys, which are much narrower.
#         # Peak/valley detection is then done relative to this local baseline,
#         # so a consistently rising or falling edge is handled correctly.
#         #
#         # Baseline window = ~4× the expected tooth pitch (must be odd).
#         n = len(x_smooth)
#         baseline_half = max(window_size * 4, n // 6)      # adapt to blade length
#         baseline_half = min(baseline_half, n // 3)         # don't exceed 1/3 of points
#         # Pad-reflect x_smooth, compute rolling median, then trim padding
#         pad = baseline_half
#         padded = np.pad(x_smooth, pad, mode='reflect')
#         baseline = np.array([
#             np.median(padded[max(0, i): i + 2 * pad + 1])
#             for i in range(n)
#         ])
#         # Where valid_mask is False, replace baseline with nearest valid value
#         # so artifact rows don't corrupt the local median at neighbouring points.
#         if not valid_mask.all():
#             valid_idx = np.where(valid_mask)[0]
#             if len(valid_idx) > 0:
#                 for i in np.where(~valid_mask)[0]:
...
#
#         # deviation of the smoothed edge from its local baseline
#         deviation = x_smooth - baseline
#
#         # ── Step 3: adaptive threshold ────────────────────────────────────────
#         # Use the IQR of the deviation (on valid points only) to set a minimum
#         # prominence threshold.  This scales naturally to the actual tooth height
#         # in the image rather than a hard-coded pixel count.
#         dev_valid = deviation[valid_mask] if valid_mask.any() else deviation
#         if len(dev_valid) > 4:
#             q1, q3 = np.percentile(dev_valid, 25), np.percentile(dev_valid, 75)
#             iqr = q3 - q1
#             # Require at least 20 % of IQR prominence; floor at 2 px
#             prominence = max(2.0, iqr * 0.20)
#         else:
#             prominence = 2.0
#
#         # ── Step 4: peak / valley detection on the deviation signal ──────────
#         peaks, valleys = [], []
#         for i in range(window_size, n - window_size):
#             if not valid_mask[i]:
#                 continue
...
#         valleys = self._filter_close_points(valleys, window_size)
#
#         # ── Debug: write mask + baseline deviation image ──────────────────────
#         if self.image is not None:
#             mask_img = img_array.copy()
#             for i, y in enumerate(y_coords.astype(int)):
#                 if 0 <= y < mask_img.shape[0]:
...
#             # draw the baseline in orange so it's easy to see in the debug image
#             for i, y in enumerate(y_coords.astype(int)):
#                 bx = int(baseline[i])
#                 if 0 <= y < mask_img.shape[0] and 0 <= bx < mask_img.shape[1]:
#                     cv2.circle(mask_img, (bx, y), 2, (0, 165, 255), -1)
...
#         #     if abs(ct.grinding_point[1] - nt.grinding_point[1])/pixels_per_mm < 2:
#         #         continue
#         #     valley_x = (ct.grinding_point[0] + nt.grinding_point[0]) / 2
#         #     valley_y = (ct.grinding_point[1] + nt.grinding_point[1]) / 2
#         #     move_x_mm = (grinder_tip[0] - valley_x) / pixels_per_mm
...
# def get_teeth_profiles():
#     """
#     Return full per-tooth data for the canvas overlay:
#       - All tooth tips (grinding_point / apex_point)
#       - Top and bottom valley coordinates
#       - Tooth ID and angle
#       - Grinder tip position
#     Runs a fresh analysis on the current frame if the camera is active.
#     Falls back to last_detection_result metadata when no camera frame is available.
#     """
#     global camera_active, last_frame, stored_grinder_tip, last_detection_result
#     try:
#         frame_to_process = None
#         if camera_active and last_frame is not None:
#             with camera_lock:
...
#
#         if frame_to_process is None:
#             # No live frame — return whatever grinder tip we have stored
#             grinder = None
#             if stored_grinder_tip:
#                 grinder = {'x': int(stored_grinder_tip[0]), 'y': int(stored_grinder_tip[1])}
#             return jsonify({'success': True, 'teeth': [], 'grinder_tip': grinder,
#                             'num_teeth': 0, 'message': 'No camera frame available'})
#
#         analyzer = SerratedBladeAnalyzer(frame_to_process)
#         analyzer.preprocess_image()
#         analyzer.detect_blade_and_grinder()
#
#         # Honour stored grinder tip
#         if stored_grinder_tip is not None:
#             analyzer.grinder_tip = stored_grinder_tip
#
#         profiles = analyzer.extract_tooth_profiles()
#
...
#         elif stored_grinder_tip:
#             grinder_tip = {'x': int(stored_grinder_tip[0]), 'y': int(stored_grinder_tip[1])}
#
#         teeth = []
#         for t in profiles:
#             teeth.append({
...
#                 'top_valley':    {'x': int(t.top_valley[0]),     'y': int(t.top_valley[1])},
#                 'bottom_valley': {'x': int(t.bottom_valley[0]),  'y': int(t.bottom_valley[1])},
#                 'angle':         round(float(t.angle), 2),
#                 'height':        round(float(t.height), 2),
#             })
#
